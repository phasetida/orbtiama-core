...
use crate::{
    ...
    deserialize::element::notes::SlideBranch,
    state::{
        element::{
...
fn tick_touch_note(time_in_second: f32, state: &mut TouchNoteState, setting: &Settings) {
    ...
    state.common.set_fixed_position(point);
...
fn tick_hold_note(time_in_second: f32, state: &mut HoldNoteState, setting: &Settings) {
    ...
    state.common.set_fixed_position(head_position);
    state.set_fixed_tail_position(tail_position);
