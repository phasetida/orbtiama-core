...
impl Location {
    ...
    pub fn get_point(&self) -> Point {
        match &self.sector {
            ...
            LocationSector::D | LocationSector::A => {
                get_position_in_circle(0.45, self.get_rad_angle())
            }
    ...
    pub fn get_rad_angle(&self) -> Angle {
        let radians = match &self.sector {
            ...
            LocationSector::D | LocationSector::E => {
                (PI / 2.0) + f32::from(self.index - 1) * (-PI / 4.0)
            }
    ...
    /// # Panics
    /// Panics when the current location or another location is not a tap location
    #[must_use]
    pub fn tap_distance_to(&self, another: &Location) -> (u8, Clockwise) {
        assert!(
            !(self.is_sensor() || another.is_sensor()),
            "internal failure: not a tap location"
        );
...
impl TryFrom<&str> for Location {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        ...
        if location_sector == LocationSector::Tap {
            ...
            let index = u8::try_from(index).map_err(|_| "not a valid index")?;
            if index == 0 || index > 8 {
...
quick_deref!(NoteTap, NoteCommon, common);
quick_deref!(NoteTouch, NoteCommon, common);
quick_deref!(NoteHold, NoteCommon, common);
quick_deref!(NoteSlide, NoteCommon, common);
