...
use crate::{
    deserialize::chart::element::chart::{Chart, NoteCollection, Timing},
    deserialize::chart::lexer::Rule,
};
    ...
    pub fn parse_chart(pairs: Pairs<Rule>) -> Chart {
        ...
        for it in root_pair.into_inner() {
            ...
            match rule {
                ...
                Rule::COMMENT | Rule::WHITESPACE | Rule::eof | Rule::eoi => {}
                _ => {
                    panic!("parser: unexpected rule {:?}", rule)
                }
            };
        }
        Chart {
