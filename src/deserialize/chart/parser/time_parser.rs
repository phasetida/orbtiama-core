...
use crate::{deserialize::chart::element::chart::Timing, deserialize::chart::lexer::Rule};
    ...
    for it in iter {
        match it.as_rule() {
            Rule::delay_duration_absolute => {
                delay = it
                    .into_inner()
                    .find(|it| it.as_rule() == Rule::second)
                    .map(|it| it.as_str())
                    .map(|it| (it, it.parse::<f32>()))
                    .map(|(str, raw)| {
                        raw.unwrap_or_else(|_| {
                            panic!("parser_duration_slide: expect number, found: {:?}", str)
                        })
                    })
                    .expect("parser_duration_slide: expect number, found nothing");
            }
